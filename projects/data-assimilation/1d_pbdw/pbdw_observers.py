...
#  x_gt = np.pi
  ...
  A_end = 1.5
  A = np.random.uniform(A_start, A_end, nbSnapshots)
#  step_position = np.random.uniform(mesh[0], mesh[len(mesh)-1], nbSnapshots)
  step_position = np.random.uniform(1, 3, nbSnapshots)
  for j in range(nbFrecuencies):
#    u_gt = u_gt + A_gt*np.sin(2*np.pi/T_gt[j]*mesh) + step(x_gt, mesh)
    u_gt = step(x_gt, mesh)
  manifold = np.zeros((len(mesh_extended), nbSnapshots))
  T = np.random.uniform(T_start, T_end, nbFrecuencies)
  for i in range(nbSnapshots):
    for j in range(nbFrecuencies):
#      manifold[:,i] = manifold[:,i] + np.sin(2*np.pi/T[j]*mesh_extended) + step(step_position[i], mesh_extended)
      manifold[:,i] = step(step_position[i], mesh_extended)
    manifold[:,i] = manifold[:,i] 

pl.figure(num=None, figsize=(fig_size_x, fig_size_y))
# plot manifold
for i in range(nbSnapshots):
  pl.plot(mesh, manifold[0:len(mesh),i], alpha=0.7)
...
if (utilde_method == "neighbourd"):
  ...
  for i in range(nbSnapshots):
    res[i] = np.linalg.norm(omega - np.matmul(rr, np.transpose(rr)*manifold[0:len(mesh),i]))/np.linalg.norm(np.matmul(rr, np.transpose(rr)*manifold[0:len(mesh),i]))
  u_tilde = manifold[0:len(mesh),np.argmin(res)]
...
if (utilde_method == "omp"):
  pl.figure(num=None, figsize=(fig_size_x, fig_size_y))
  v_omp = np.zeros((len(mesh), nbModes))
  U_omp = np.zeros((len(mesh), nbModes))
  ...
  for i in range(nbSnapshots):
    ...
    for j in range(nbMeasures):
      measure = measure + rr[:,j]*np.matmul(np.transpose(manifold[0:len(mesh),i]),rr[:,j])
    z[:,i] = measure / np.linalg.norm(measure)

  # run OMP 
  Pi_Wm_Ui = np.zeros((len(mesh), nbModes))
  id_or = -1*np.ones(nbModes-1).astype(int)
    ...
    for j in range(nbSnapshots):
      ...
      if (res > greedy):
        if (isinside(j, id_or) == 0):
          greedy = res
          id_or[i-1] = j
    U_omp[:,i] = manifold[0:len(mesh), id_or[i-1]] 
...
for j in range(1,nbModes):
  G = np.matmul(np.transpose(rr), U[:,0:j])
  LHS = np.matmul(np.transpose(G), G)
  RHS = np.matmul(np.transpose(U[:,0:j]), omega_eta)
  c = np.linalg.solve(LHS, RHS)
  for i in range(j):
...
pl.plot(mesh, u_tilde, label='$\\tilde{u}$')
pl.plot(mesh, u_star[:,np.argmin(error_l2)+1], label='$u^*$ (' + str(np.argmin(error_l2)+1) + ' modes)')
#pl.xlim(0, T)
pl.ylim(min(u_gt), max(u_gt))
show(1)
