  ...
  for i in sims:
    for k in range(10,50,10):
      for noise_level in noise:
        ...
        UStimeStart=0.0
        UStimeEnd=0.19*60.0/hr
        dirResults="../../data/reconstruction/u_H1/noise/sim" + wildcard(i) + "/" + str(k) + "modes/cls_noise_" + str(int(noise_level)) + "_it" + str(j) + "/"
        ...
        call("cp data_imrp " + dataFile, shell=True)
        ...
        if n_procs == 1:
          cmd = "./imrp.exe " + dataFile
        else:
          cmd = "./imrp.exe " + dataFile + " > " + dirResults + "imrp.log"
