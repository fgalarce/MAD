  ...
  for i in sims:
    for k in range(10,50,10):
      for noise_level in noise:
        ...
        UStimeStart=0.1*60.0/hr
        UStimeEnd=0.2*60.0/hr
      
        dirResults="../../data/reconstruction/up_H1/noise/sim" + wildcard(i) + "/" + str(k) + "modes/noise_" + str(int(noise_level)) + "_it" + str(j) + "/"
        ...
        if n_procs == 1:
          ...
          cmd = "./mixed_pbdw.exe " + dataFile + " > " + dirResults + "pbdw.log"
...
if n_procs == 1:
  ...
  p.map(loopIter, range(noiseIter))
