...
def loop(n):
  for j in range(nbSimulations):
    dirResults="../../../../data/rectangle/pbdw/test" + wildcard(j) + "/n" + str(n) + "/"
    dirField="../../../../data/rectangle/test/sim" + wildcard(j) 
    call("mkdir -p " + dirResults, shell=True)
    dataFile = dirResults + "parameters"
    call("cp parameters_rectangle " + dataFile, shell=True)
    setParameter("dirResults", dirResults, dataFile) 
    setParameter("dirSyntheticField", dirField + "/solid_disp" , dataFile) 
    setParameter("innerProduct", "l2" , dataFile) 
    setParameter("nbModes", n, dataFile) 
    
    if n_procs == 1:
        cmd = mpi + " -np 72 ./pbdw.exe " + dataFile
    else:
        cmd = mpi + " -np 1 ./pbdw.exe " + dataFile + " > " + dirResults + "/pbdw.log"
