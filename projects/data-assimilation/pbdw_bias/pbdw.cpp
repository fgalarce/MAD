...
vector<double> noise(double pure_signal, Parameters par){
...
int main(int argc, char *argv[]){
  ...
  for (int iteration = par.start(); iteration < par.end(); iteration++ ){
    ...
    if (world_rank == 0) cout << "PBDW: Computing auxiliar solution." << endl;
    Vec u_tilde = zeros(nbDofs);
    vector<double> ci = stl(c);
    for (int i = 0; i < n; i++){
      VecAXPY(u_tilde, ci[i], Vn.basis(0)[i]);
    }

    Vec eta = vec(measures.nbMeasures());
