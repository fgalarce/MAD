...
def loop(idMesh):
  for j in range(nbTargets):
    ...
    dirResults="../../data/venturi/mor/mesh" + wildcard(idMesh) + "/targets/" + wildcard(j) + "/"
    call("mkdir -p " + dirResults, shell=True)

    # Template
    templateGeometry="../../data/venturi/mesh/venturi" + wildcard(idMesh) + ".mesh"
    surfaceMapping="../../data/venturi/mappings/" + wildcard(idMesh) + "/lddmm_target" + wildcard(j) + ".vct"
    dirModel="../../data/venturi/mor/mesh" + wildcard(idMesh) + "/"

    # Target
    geometryData="../../data/venturi/mesh/target" + wildcard(j) + ".mesh"
    surfaceMappingInverse="../../data/venturi/mappings/target" + wildcard(j) + "/lddmm_target" + wildcard(idMesh) + ".vct"
