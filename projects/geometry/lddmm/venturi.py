...
def loop(idMesh):
  for j in range(nbMeshes):
    ...
    dirResults="../../../../data/venturi/mappings/" + wildcard(idMesh) + "/"
    call("mkdir -p " + dirResults, shell=True)
    source= "../../../../data/venturi/venturi" + wildcard(idMesh) + ".mesh"
    target="../../../../data/venturi/venturi" + wildcard(j) + ".mesh"

    run = "python3.6 LDDMM.py -scale=0.1 -save_folder=" + dirResults + " -name=" + ensName + " -source=" + source + " -target=" + target
