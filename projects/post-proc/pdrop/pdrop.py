...
def loop(j):
  ...
  targetFolder = "../../data/targets/sim" + wildcard(j) + "/"
  ...
  dirResults="../../data/reconstruction/up_H1/sim" + wildcard(j) + "/"

  call("mkdir -p " + dirResults, shell=True)
  dataFile = dirResults + "data_imrp"
  ...
  if n_procs == 1:
      cmd = "./pdrop.exe " + dataFile + " " + str(j)
  else:
      cmd = "./pdrop.exe " + dataFile + " " + str(j) + " > " + dirResults + "/imrp.log"
