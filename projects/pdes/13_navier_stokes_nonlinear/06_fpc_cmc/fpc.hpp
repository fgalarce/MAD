...
//      bd.DirichletNormalParaboloid(par.inlet(), inlet, range(geo.dimension()));
      ...
      bd.Dirichlet(par.inlet(), inlet, 1);
//      bd.Dirichlet(2, outlet, 1);
...
//      bd.NeumannNormalConstant(par.outlets()[0], -1.0, 0);
      bd.block(A, "symmetric");
      bd.block(b, "symmetric");
    }

    void solve(Mat A, Boundary bd, Vec u, double t, NS & ns, Vec PesoPropio){  
...
//        Vec b_iter = ns.assembleRHS();
//        VecAXPY(b_iter, 1.0, PesoPropio);
//        applyBC(C, b_iter, t);
