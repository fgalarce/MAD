...
//  return -par.amplitude()*sin(2*PI*t)*sin(2*PI*t)/1.237744e-01;
////  double tstar = t/(par.distalResistances()[0]*par.capacitances()[0]);
////  vector<double> bc(1,par.amplitude()*sin(tstar/2.0)*sin(tstar/2.0)/12.566370614);
//}
...
class Tube{
  ...
  public:
    ...
    void applyBC(Vec u, Mat A, Vec b, double t){
      ...
      bd.time(t); 
      bd.Dirichlet(par.inlet(), zero, 0);
      bd.Dirichlet(par.inlet(), zero, 1);
