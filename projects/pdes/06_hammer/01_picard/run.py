...
Lenght = arange(50,220, 20)
wave_velocity = arange(400,1300,100)

def loop(j):
  for B in Bi:
    for l in Lenght:

      dirResults="./Lenght_" + str(int(l)) + "/Bi_" + str(int(B*10)) + "/c_" + str(int(wave_velocity[j])) + "/"
      ...
      setParameter("wave_velocity", wave_velocity[j], dataFile) 
      setParameter("length", l, dataFile) 
...
if n_procs == 1:
  for j in range(len(wave_velocity)):
    loop(j)

else:
  p = Pool(processes = n_procs)
  p.map(loop, range(len(wave_velocity))) 
