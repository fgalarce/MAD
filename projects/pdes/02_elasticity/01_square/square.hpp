...
vector<double> abajo(vector<double> x, double t, Parameters par){
  vector<double> bc(par.nbDofsPerNode()[0], 0.0);
  return bc;
}

vector<double> arriba(vector<double> x, double t, Parameters par){
  vector<double> bc(par.nbDofsPerNode()[0], 0.0);
  bc[1] = par.amplitude();
  return bc;
}
...
class Square{
  ...
  public:
    ...
    void applyBC(Mat A, Vec b){
      bd.Dirichlet(par.bottom(), abajo);
      bd.Dirichlet(par.top(), arriba);
      bd.block(A);
      bd.block(b);
    }
