...
vector<double> pulso(vector<double> x, double t, Parameters par){
  vector<double> bc(par.nbDofsPerNode()[0], 0.0);
  if (t < par.period()){
    bc[0] = par.amplitude()*sin(2*PI*t/par.period() + par.timeStep());
  }
//  int iteration = (int)floor(t/par.timeStep());
//  bc[0] = importdata1D("./disp_armenia.txt", "shut_up")[iteration]/100;
  return bc;
...
class Soil{
  ...
  public:
    ...
    void applyBC(Mat A, Vec b, double t){
      ...
      bd.Dirichlet(8, pulso);
      bd.block(A);
      bd.block(b);
      int iteration = (int)floor(t/par.timeStep());
//      cout << importdata1D("./disp_armenia.txt", "shut_up")[iteration]/100;
    }
