...
int main(int argc, char *argv[]){
  ...
  NaturalConvection ns;
  ns.initialize(par, geo, boundary);

  FPC fpc;
  fpc.initialize(boundary, geo, par);

  int nbDofs = io.nbVertices()*(par.nbDofsPerNode()[0] + par.nbDofsPerNode()[1] + par.nbDofsPerNode()[2]);
  ...
  for (double t : range(0, par.nbIterations()*par.timeStep(), par.timeStep())){
    ...
    fpc.solve(ns, A, boundary, u, calculus, t, io);
    ns.update(u, t);
