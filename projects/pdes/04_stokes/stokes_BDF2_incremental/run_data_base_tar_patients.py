...
def loop(j):
  ...
  preDirResults="../../data/venturi/dictionary/target_mesh" + wildcard(j) + "/" 
  call("mkdir -p " + preDirResults, shell=True)
  preDataFile=preDirResults + "/parameters" 
  call("cp parameters " + preDataFile, shell=True)

  setParameter("geometryData", "../../data/venturi/mesh/target" + wildcard(j) + ".mesh", preDataFile) 

  for i in range(nbSims):

    dirResults = preDirResults + "/target" + wildcard(i) + "/" 
