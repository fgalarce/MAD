...
//  double r = 1.0;
//  double T = 4.0/5.0;
  double A = par.inlet_u0();
//  double shape = 100 * 1.0/ 0.41 / 0.41 * x[1]*x[2]*( 0.41 - x[1] ) * ( 0.41 - x[2] );
...
//            double Kij = fe.stiffness_symmetric(i,j);
  ...
  for (double t : range(0, par.nbIterations()*par.timeStep(), par.timeStep())){
    ...
    for (int tetraId = 0; tetraId < elements.size(); tetraId++){ /* loop on elements */
      ...
      if (tetraId % (elements.size() / 5) == 0){
        if (world_rank == 0) cout << "  Elementary matrix for simplex: " << tetraId << "/" << elements.size() - 1 << endl;
      }
        ...
        for (int i = 0; i < nbNodesPerElement; i++){
          ...
          for (int comp = 0; comp < par.nbDofsPerNode()[0]; comp++){
            if (par.nbDofsPerNode()[0]*simplex[i]+comp >= m && par.nbDofsPerNode()[0]*simplex[i]+comp < n){
              ...
              MatSetValue(C, par.nbDofsPerNode()[0]*simplex[i]+comp, par.nbDofsPerNode()[0]*simplex[j]+comp, Aij, ADD_VALUES);
            }
          }
        }
      }
