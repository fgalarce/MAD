...
vector<double> zero(vector<double> x, double t){
...
//  double mu = par.youngModulus() * par.poissonRatio() / ((1.0 + par.poissonRatio())*(1.0 - 2.0*par.poissonRatio()));
  ...
  for (int finiteElementId = 0; finiteElementId < geo.tetrahedron()[0].size(); finiteElementId++){ /* loop on finiteElement */
    ...
    for (int nodeId = 0; nodeId < nbNodesPerElement; nodeId++){
      ...
      mean_point = coordinates[nodeId] + mean_point;
    }
    mean_point = mean_point * (1.0 / geo.dimension());

    double porosity;
//    if (   mean_point[0] >= par.x_range()[0] && mean_point[0] < par.x_range()[1]
//        && mean_point[1] >= par.x_range()[0] && mean_point[1] < par.x_range()[1] 
//        && mean_point[2] >= par.x_range()[0] && mean_point[2] < par.x_range()[1] ){
//      porosity = par.porosity2();
//    } else {
...
//        && mean_point[2] >= par.x_range()[0] && mean_point[2] < par.x_range()[1] ){
    ...
    double r0 = sqrt(mean_point[0]*mean_point[0] + mean_point[1]*mean_point[1] + mean_point[2]*mean_point[2]);
    if (r0 <= par.x_range()[0]){
      porosity = par.porosity2();
    } else {
      porosity = par.porosity();
      ...
      for (int j = 0; j < nbNodesPerElement; j++){
        for (int comp = 0; comp < nbDofsPerNode; comp++){
          if (nbDofsPerNode*elementNodes[i]+comp >= m && nbDofsPerNode*elementNodes[i]+comp < n){
            double Bij = finiteElement.mixed(i,j,comp);
          ...
          if (nbDofs_uSol + nbDofsPerNode*elementNodes[i]+comp >= m && nbDofs_uSol + nbDofsPerNode*elementNodes[i]+comp < n){
            double Bij = finiteElement.mixed(i,j,comp);
            double Mij = finiteElement.mass(i,j);
            MatSetValue(A, nbDofs_uSol + nbDofsPerNode*elementNodes[i]+comp, nbDofs_uSol + nbDofsPerNode*elementNodes[j]+comp, 1.0/porosity * Mij, ADD_VALUES);
        ...
        if (nbDofs_uSol + nbDofs_uFlu + elementNodes[i] >= m && nbDofs_uSol + nbDofs_uFlu + elementNodes[i] < n){
          double Kij = finiteElement.stiffness(i,j);
          double Mij = finiteElement.mass(i,j);
          MatSetValue(M, nbDofs_uSol + nbDofs_uFlu + elementNodes[i], elementNodes[j], Mij, ADD_VALUES);
          MatSetValue(A, nbDofs_uSol + nbDofs_uFlu + elementNodes[i], nbDofs_uSol + nbDofs_uFlu + elementNodes[j], finiteElement.size()*finiteElement.size() * Kij, ADD_VALUES);
        }
      }
    }
  }
