...
//  io.writeState(u0, 0.0);
  ...
  for (int partId = 0; partId < geo.elements.size(); partId++){
    for (int feId = 0; feId < geo.elements[partId].size(); feId++){ /* loop on elements */
      ...
      fe.computeSize();
      double tau_stab = fe.size()*fe.size();
        ...
        for (int i = 0; i < nbNodesPerElement; i++){
          for (int comp = 0; comp < par.nbDofsPerNode()[0]; comp++){
            ...
            if (nbDofsSol + nbDofsPerNode*simplex[i]+comp >= m && nbDofsSol + nbDofsPerNode*simplex[i]+comp < n){
              ...
              code = MatSetValue(A, nbDofsSol + nbDofsPerNode*simplex[i]+comp, nbDofsSol + nbDofsPerNode*simplex[j]+comp, Mij, ADD_VALUES); CHKERRQ(code);
              double Bij = fe.mixed(i,j,comp);
              code = MatSetValue(A, nbDofsSol + nbDofsPerNode*simplex[i]+comp, nbDofsSol + nbDofsFlu + simplex[j], -K_darcy*Bij, ADD_VALUES); CHKERRQ(code);
              /* Residual stabilization  */
              code = MatSetValue(A, nbDofsSol + nbDofsPerNode*simplex[i]+comp, nbDofsSol + nbDofsPerNode*simplex[j]+comp, tau_stab * Mij, ADD_VALUES); CHKERRQ(code);
              code = MatSetValue(A, nbDofsSol + nbDofsPerNode*simplex[i]+comp, nbDofsSol + nbDofsFlu + simplex[j], K_darcy * tau_stab * Bij, ADD_VALUES); CHKERRQ(code);
            }

            /* Mass conservation */
            if (nbDofsSol + nbDofsFlu + simplex[j] >= m && nbDofsSol + nbDofsFlu + simplex[j] < n){
              double Bij = fe.mixed(i,j,comp);
              MatSetValue(M_sol_2, nbDofsSol + nbDofsFlu + simplex[j], nbDofsPerNode*simplex[i]+comp, -Bij/par.timeStep(), ADD_VALUES);  /* B^T */
              MatSetValue(A, nbDofsSol + nbDofsFlu + simplex[j], nbDofsSol + nbDofsPerNode*simplex[i]+comp, -Bij, ADD_VALUES);  /* B^T */
              double Kij = fe.stiffness(i,j);
              /* Residual stabilization */
              code = MatSetValue(A, nbDofsSol + nbDofsFlu + simplex[j], nbDofsSol + nbDofsFlu + simplex[i], K_darcy * K_darcy * tau_stab * Kij, ADD_VALUES); CHKERRQ(code);
              code = MatSetValue(A, nbDofsSol + nbDofsFlu + simplex[j], nbDofsSol + nbDofsPerNode*simplex[i]+comp, K_darcy * tau_stab * Bij, ADD_VALUES); CHKERRQ(code);  /* -B*/ 
            }
          }
        }
      }
  ...
  boundary.Dirichlet(par.inlet(), wall, 0);
  boundary.Dirichlet(par.outlet(), outlet, 0);
  boundary.Dirichlet(par.outlet(), outlet, 1);
  for (int i : par.walls()){
    boundary.Dirichlet(i, wall, 1);
//    boundary.DirichletComp(1, i, wall, 0); /* confined , only axial movement allowed */
  }
