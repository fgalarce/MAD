...
vector<double> inlet_shape(vector<double> x, double t, Parameters par){
  ...
  double r = 1.0;
  bc[1] = 60*(1.0 - x[0]*x[0] - x[2]*x[2])*sin(5*PI/2*t);
//  bc[1] = 60*(1.0 - x[0]*x[0] - x[2]*x[2]);
  return bc;
}
...
int main(int argc, char *argv[]){
  ...
  PetscBool reuse_prec = PETSC_FALSE;
  ...
  code = KSPSetInitialGuessNonzero(ksp, PETSC_TRUE); CHKERRQ(code);
  code = KSPSetTolerances(ksp, 1e-09, 1e-50, 20000, 20000);
